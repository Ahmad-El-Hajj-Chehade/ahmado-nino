...
const KaufleutenAlbum = () => {
  const [imageFilenames, setImageFilenames] = useState([]);
  const [lightboxImageIndex, setLightboxImageIndex] = useState(null);

  useEffect(() => {
    // Fetch the JSON file
    fetch(`${process.env.PUBLIC_URL}/kaufleuten_21_12_24/imageList.json`)
      ...
      <section className="album-detail">
        ...
        <div className="album-grid">
          {imageFilenames.map((filename, index) => {
            const imagePath = `${process.env.PUBLIC_URL}/kaufleuten_21_12_24/JPEG/${filename}`;
            return (
                <div key={index} className="album-item" onClick={() => openLightbox(index)}>
                  <img src={imagePath} alt={`Kaufleuten ${index + 1}`} loading="lazy" />
                </div>
            );
          })}
        </div>
        ...
        {lightboxImageIndex !== null && (
            ...
            >
              ...
              <img
                  ...
                  src={`${process.env.PUBLIC_URL}/kaufleuten_21_12_24/JPEG/${imageFilenames[lightboxImageIndex]}`}
                  alt="Enlarged"
              />
...
export default KaufleutenAlbum;
