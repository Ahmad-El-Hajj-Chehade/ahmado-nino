...
const ShineFestAlbum = () => {
  const [imageFilenames, setImageFilenames] = useState([]);
  const [lightboxImageIndex, setLightboxImageIndex] = useState(null);

  useEffect(() => {
    // Fetch the JSON file
    fetch(`${process.env.PUBLIC_URL}/shinefest_15_03_2025/imageList.json`)
  ...
  return (
      <section className="album-detail">
        <h1>Shinefest 15.03.2025</h1>
        <div className="album-grid">
          {imageFilenames.map((filename, index) => {
            const imagePath = `${process.env.PUBLIC_URL}/shinefest_15_03_2025/JPEG/${filename}`;
            return (
                <div key={index} className="album-item" onClick={() => openLightbox(index)}>
                  <img src={imagePath} alt={`Shinefest ${index + 1}`} loading="lazy" />
                </div>
            );
          })}
        </div>
        ...
        {lightboxImageIndex !== null && (
            ...
            >
              ...
              <img
                  ...
                  src={`${process.env.PUBLIC_URL}/shinefest_15_03_2025/JPEG/${imageFilenames[lightboxImageIndex]}`}
                  alt="Enlarged"
              />
