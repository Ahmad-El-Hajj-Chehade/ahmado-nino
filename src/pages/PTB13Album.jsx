...
const PlazaAlbum = () => {
    ...
    useEffect(() => {
        // Fetch image list
        fetch(`${process.env.PUBLIC_URL}/plaza_29_03_25/imageList.json`)
    ...
    return (
        <section className="album-detail">
            <h1>13 Years PLAZE TO BE</h1>
            <div className="album-grid">
                {imageFilenames.map((filename, index) => {
                    const imagePath = `${process.env.PUBLIC_URL}/plaza_29_03_25/JPEG/${filename}`;
                    return (
                        <div key={index} className="album-item" onClick={() => openLightbox(index)}>
                            <img src={imagePath} alt={`Plaza ${index + 1}`} loading="lazy" />
                        </div>
                    );
                })}
            </div>
            ...
            {lightboxImageIndex !== null && (
                <div className="lightbox show" onClick={closeLightbox} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
