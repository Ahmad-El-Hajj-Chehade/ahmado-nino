...
const PrimesideAlbum = () => {
  const [imageFilenames, setImageFilenames] = useState([]);
  const [lightboxImageIndex, setLightboxImageIndex] = useState(null);

  useEffect(() => {
    // Fetch the JSON file
    fetch(`${process.env.PUBLIC_URL}/primeside/imageList.json`)
  ...
  return (
      <section className="album-detail">
        <h1>Primeside Festival</h1>
        <div className="album-grid">
          {imageFilenames.map((filename, index) => {
            const imagePath = `${process.env.PUBLIC_URL}/primeside/JPEG/${filename}`;
            return (
                <div key={index} className="album-item" onClick={() => openLightbox(index)}>
                  <img src={imagePath} alt={`Primeside ${index + 1}`} loading="lazy" />
                </div>
            );
          })}
        </div>
        ...
        {lightboxImageIndex !== null && (
            ...
            >
              ...
              <img
                  ...
                  src={`${process.env.PUBLIC_URL}/primeside/JPEG/${imageFilenames[lightboxImageIndex]}`}
                  alt="Enlarged"
              />
